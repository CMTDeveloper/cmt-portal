...
const row = (over: Partial<RosterPersonCsvRow>): RosterPersonCsvRow => ({
  ...
  type: 'Child', grade: '3', level: 'Level 3', location: 'Brampton', programs: 'Bala Vihar', payment: 'paid', participating: 'yes', ...over,
});

describe('rosterToCsv', () => {
  it('emits a header row even with no data', () => {
    expect(rosterToCsv([])).toMatch(/^familyName,fid,legacyFid,memberName,type,grade,level,location,programs,payment,participating$/);
  });
