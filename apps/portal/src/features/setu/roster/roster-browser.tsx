...
// The roster's opening view, named once so the initial state and Reset cannot
// drift apart - a Reset that restores something the screen never opens on is
// worse than no Reset, because it silently moves the user somewhere new.
const DEFAULT_PAYMENT: RosterPayment = 'paid';
const DEFAULT_ENGAGEMENT: RosterEngagement = 'enrolled';
...
function RosterContent({ year, locationOptions }: { year?: string; locationOptions: string[] }) {
  ...
  const [payment, setPayment] = useState<RosterPayment | null>(DEFAULT_PAYMENT);
  const [engagement, setEngagement] = useState<RosterEngagement | null>(DEFAULT_ENGAGEMENT);
  ...
  // Every control Reset owns, including the search box. `?year=` is deliberately
  // NOT one of them: the school year is a scope selected in its own bar, not a
  // filter chip, and clearing it would move the user to a different year's data
  // while they were only trying to widen the current one.
  const isDefault =
    ...
    <div className="col" style={{ gap: 16 }}>
      ...
      <div className="between" style={{ gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {/* Deliberately OUTSIDE the `!searchActive` gate above. That gate
              unmounts the whole filter grid the moment the search box has text,
              so a Reset placed inside it would vanish exactly when the only
              thing left to clear is the search. */}
          {!isDefault && (
            <button
              type="button"
              onClick={resetFilters}
              style={{
                minHeight: 36, padding: '0 12px', fontSize: 13, fontWeight: 600,
