...
import { activateSchoolYear } from '@/features/setu/rollover/activate-school-year';
...
export async function POST(req: Request) {
  ...
  // Atomic dual-write: the live year and seva's year flip together (or not at
  // all) inside one transaction, so they can never drift if one write fails.
  const { config, sevaYear } = await activateSchoolYear(db, { toYear, actorMid });

  revalidateTag('school-year', 'max'); // live-year badges everywhere
  revalidatePath('/admin/school-year'); // the Year center
  return NextResponse.json({ config, sevaYear }, { status: 200 });
}
