...
Was the bare Firestore document id (`U78VVE09p1jah98WUTj5`), which identifies the payment perfectly and the family not at all. Vaibhav, 2026-08-04, from a live Stripe call: *"if this is generated by app, then can you replaced that with familyId instead?"* It cannot simply BECOME the family id - a family makes many payments and this is the only link back to one record - so it now carries both, family first.

**Mobile action:** none required, unless the app parses or displays this value. If it does, do NOT treat it as a document id any more - use `did` for that. Old donations keep their old bare-id value; nothing was rewritten, so both formats exist in production and any parser must tolerate both.
...
**Mobile action, if the app offers a family-facing member delete: it now FAILS.** `DELETE /api/setu/members/{mid}` returns **403 `families-cannot-remove-members`** for every caller as of 2026-08-04 (it previously accepted any family-manager session, and called the write core with no actor, so a family delete left no audit row at all). Withdraw the affordance and point at the reversible control instead - `PATCH /api/setu/members/{mid}` with `{ participation: 'inactive' }`, which keeps the record and past attendance and can be undone. The web copy is now "Not taking part - hide from class lists" rather than "No longer participating", which Vaibhav found unclear.
