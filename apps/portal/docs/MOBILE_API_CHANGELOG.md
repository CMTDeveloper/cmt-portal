...
- **The staff route `PATCH /api/welcome/families/[fid]` behaves identically** for `location` as of `d9c1e05` (same allow-list check, same flag clear). It is welcome-team-only and has **no mobile mirror** - noted so the contract-sync cron does not read the silence as an oversight.
...
- **`FamilyDocSchema` gains `locationNeedsConfirmation: boolean | null | undefined`.** It appears in the `family` object of **`GET /api/setu/family`** and, added in `d9c1e05`, of **`GET /api/setu/dashboard`** (whose `family` object is a deliberately narrower hand-built projection, not the full `FamilyDoc` - it was missing there when this entry was first written). Purely additive - every existing field is unchanged, and an older client that ignores it still works.
