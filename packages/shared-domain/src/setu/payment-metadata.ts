...
/**
 * Stripe's `client_reference_id`: the family FIRST, then the record.
 *
 * Vaibhav, 2026-08-04, looking at a live Stripe call: *"I am getting the
 * following in Stripe call... 'client_reference_id': 'U78VVE09p1jah98WUTj5', if
 * this is generated by app, then can you replaced that with familyId instead?"*
 *
 * We do generate it - it was the bare Firestore document id of the donation (or
 * the pledge), which identifies the payment perfectly and identifies the FAMILY
 * not at all. But it cannot simply BECOME the family id: a family makes many
 * payments, and the id is the only thing tying a Stripe row back to one portal
 * record. Support's question is "which payment is this?"; his is "whose is it?".
 * Both are answerable, so answer both:
 *
 *     FID-5006-U78VVE09p1jah98WUTj5
 *     ^^^^^^^^ readable at a glance in the dashboard
 *              ^^^^^^^^^^^^^^^^^^^^ the exact record
 *
 * Nothing in this codebase parses the value back - the return URLs carry `did`
 * and `pledge` as their own query parameters, and the Stripe lookups go by
 * session id. So the format is free to be human-first. Stripe allows 200
 * characters; this is ~30.
 */
export function buildClientReferenceId(args: { familyLabel: string; recordId: string }): string {
  return `${args.familyLabel}-${args.recordId}`;
}
