...
- **Phase 3 — Roster**: ✅ Shipped (2026-06-09). `/welcome/roster` (welcome-team + admin) replaces single-shot family search: browse all Setu families (name-ordered, fid-cursor pagination), search-as-filter (reuses `searchFamilies`), location + program filters, payment chip, drill into the existing `/welcome/family/[fid]`. Flat one-row-per-person CSV export via bulk collectionGroup reads (`features/setu/roster/build-csv-rows.ts` — fast, no new indexes). Read-only migration-completeness check vs the legacy 715b8 RTDB roster (`reconcile-migration.ts` + `GET /api/welcome/families/migration-status`) — the **"Migration status" card was removed from `/welcome/roster` on 2026-08-03** (a one-off cutover tool on a daily-use screen); the endpoint and helper stay as a diagnostic with no UI. New `GET /api/welcome/families` + `/api/welcome/families/migration-status` (canAccessRoute rule). Two UAT-deployed indexes back the filters: `enrollments(programKey,status)` collection-group + `families(location,name)`. `/welcome` now redirects to `/welcome/roster`. Verified by `e2e/setu/admin/roster.spec.ts` against deployed UAT.
- **Phase 4 — Reports hub**: ✅ Shipped (2026-06-09). `/welcome/reports` (welcome-team + admin) hub. **The donations report/summary was REMOVED 2026-07-15** (CMT decision — no collective financial info in the reports hub); `REPORT_KINDS` is now `['enrollment','attendance']`, the `DonationsReport` type/schema + `donations-report.ts` + `donationsReportToCsv` are gone, and `/api/welcome/reports/donations` now returns 400. Remaining cards: enrollment headcounts (per program + per level), attendance summary (per level/program present/absent + rate, with a date-range control), and the legacy check-in/guest CSVs (admin-only, reusing the existing `/api/check-in/admin/reports/[kind]`). Unified read-only API `GET /api/welcome/reports/{enrollment,attendance}` (`?format=json|csv`), welcome-team-gated. Aggregations use bulk reads — **no new Firestore indexes**. `/check-in/admin/reports` redirects to `/welcome/reports`. v1 deviations: `location` is not a report filter; attendance program labels are title-cased from the slug. Verified by `e2e/setu/admin/reports.spec.ts` against deployed UAT. **The admin-section revamp (Phases 1–4) is now complete.**

**Welcome-team role scope (2026-08-03, from production feedback):** the role is
**roster + visitors, and nothing else**. Vaibhav: *"They need to be able to update
roster and visitors. No other access needed."* Levels, Seva, Prasad, Reports, the
class-calendar writes, teacher assignment, per-level teachers and the enrollment
payment override are now **admin-only** (teacher assignment + teacher search +
per-level teachers keep `coordinator`). Admins lose nothing — `isWelcomeTeam()`
returns true for admin, so every removed capability stays theirs. `/docs` is a
deliberate exception: 5 guides in the docs registry are tagged `welcome-team`.
The whole grant is pinned as an allow/deny table in
`packages/shared-domain/src/__tests__/can-access-route.test.ts`
("the welcome-team grant, in full") — **change that table, not just a rule**, and
note that the `/welcome/*` catch-all now defaults to `isAdmin`, so a NEW
`/welcome` page is admin-only until granted explicitly. Pages that welcome-team
lost carry their own second gate via `denyUnlessAdmin()` (`lib/require-admin-page.tsx`),
because the welcome LAYOUT still admits welcome-team for the roster.

**Prasad module status:** ✅ Shipped (2026-06-10). One prasad Sunday per family per school year, assigned by the youngest child's birthday month with cap-balanced spill (pure engine `@cmt/shared-domain/setu/prasad-engine`, rollover-pattern preview→publish at `/admin/prasad`). Revised 2026-06-11 to **propose→confirm** (admin-team feedback): publish writes `status:'proposed'` proposals, families confirm in place or pick any open Sunday at `/family/prasad`, admin assigns stragglers per-row or via "Assign all unconfirmed"; reminders are status-aware (confirm-nudge vs bring-prasad copy). Spec: `docs/superpowers/specs/2026-06-10-prasad-propose-confirm-design.md`. Family self-serve moves (7-day lock, transactional cap check) at `/family/prasad`; welcome-team day-of list at `/welcome/prasad`; 7d/2d email+SMS reminders via daily cron (`PRASAD_REMINDER_CRON_ENABLED` gates sends). Collections `prasadAssignments`/`prasadConfig`; member field `birthMonth` (1–12, backfilled from legacy `dob_m`); calendar flag `prasadNeeded`. Spec: `docs/superpowers/specs/2026-06-10-prasad-module-design.md`. Deferred v1: family-form birth-month select; Scarborough calendar entry (operational, via `/admin/calendar`). `CURRENT_PRASAD_PIDS` must be bumped at each school-year rollover.
...
7. **Every user-facing route gets a deployed-UAT E2E with a realistic, multi-instance fixture in its *active* state.** A route with no end-to-end test is untested — `/family/seva` shipped with zero E2E and 500'd in prod. The fixture must look like a real account (N≥2 of anything plural, config switched **on**, data present), not the minimal happy-path one. Mocked unit tests + fake-firestore verify code-internal consistency, NOT the integration layer (Next `use cache`, router/redirect gates, Firestore index enforcement) where these bugs live — so finish with a walkthrough against deployed UAT. See the `verifying-setu-changes-in-uat` skill.

8. **Audit Firestore composite indexes on any query change.** Every `.where().orderBy()` / multi-`where` query needs a matching entry in `firestore.indexes.json`; fake-firestore is index-blind, so green tests prove nothing about indexes. A 3-field `(a,b,c)` index does NOT serve `where(a).orderBy(c)` — two query shapes usually need two indexes (this caused the `/family/seva` 500: `(sevaYear,status,date)` existed but `(sevaYear,date)` didn't). Deploy to UAT only (`--project chinmaya-setu-uat`, never `--force`, never prod). See the `auditing-firestore-indexes` skill.
   - **A collection-group query needs an index for ANY `where`, including a single-field one** — those need a `fieldOverrides` entry, not an `indexes` one, and Firestore's automatic single-field indexes are COLLECTION scope only. `/welcome/levels` 500'd on every request from the day it shipped (prod digest `399933007`) because `collectionGroup('enrollments').where('status','==','active')` had no `enrollments.status` COLLECTION_GROUP override in either project. **Prefer dropping the filter over adding the index:** an UNFILTERED `collectionGroup(...).get()` needs no index at all, and filtering in memory is what `roster/build-csv-rows.ts`, `roster/report-dataset.ts` and now `teacher/welcome-read.ts` all do over this same data. That also avoids an index deploy against shared prod.

9. **Leave a redirect-gated screen via a HARD navigation, never `router.push`.** A soft push into a `redirect()` gate (e.g. the `/family` profile-completion gate) that reads a stale `use cache` value bounces back to the SAME route and React preserves the component's state — `/complete-profile` stranded on "Saving…" forever. Use `window.location.assign(...)` to leave such a screen; the full load re-runs the gate server-side on fresh data. And don't make a client decision from a read you just invalidated with `revalidateTag` (it's background/stale-tolerant) — trust the write you just made.
