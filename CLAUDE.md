...
- **Phase 2 — Users & Roles**: ✅ Shipped. `/admin/users` (admin-only) replaces the 3 fragmented grant screens. Dual-path role model merged via `features/setu/auth/manage-roles.ts` (`grantRole`/`revokeRole`/`listStaff`); `GET/POST /api/admin/users` + `DELETE /api/admin/users/roles` (self-lockout + last-admin guards). Needed the `members.mid` collection-group field-override index (UAT-deployed).
...
- **Phase 4 — Reports hub** (`/welcome/reports`): ⏳ Not yet built. Enrollment/attendance summaries (welcome-team + admin), donations summary (admin-only), legacy check-in CSV.
